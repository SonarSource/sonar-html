 ...
 */
@Check(key = "MaxLineLengthCheck", title = "MaxLineLengthCheck", description = "Checks the lenght of a line", priority = Priority.MINOR, isoCategory = IsoCategory.Maintainability)
public class MaxLineLengthCheck extends AbstractPageCheck {
  ...
  private void check(Node node, int newlines) {
    if (currentLineLength > maxLength) {
      createViolation(node.getStartLinePosition() + newlines);
    }
    currentLineLength = 0;
  }
